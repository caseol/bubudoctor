...
function getInputHtml(currentColumnIndex, settings, oldValue) {
    ...
    switch (inputType) {
        case "list":
            input.html = startWrapperHtml + "<select class='" + inputCss + "' onkeydown='if (\" +listenToKeys +\"){key=(event.which || event.keyCode);if (key==9||key==13){$(this).updateEditableCell(this)}else if(key==27){$(this).cancelEditableCell(this)}}' onchange='$(this).updateEditableCell(this);'>";
        ...
        jQuery(".date-time-picker").datepicker("destroy");
            startWrapperHtml = "" +
            "<div class='input-group date-time-picker' id='datepicker-appointment-date' data-target-input='nearest'>" +
            "<input id='ejbeatycelledit' class='date-time-picker " + inputCss + "' onkeydown='if (" +  listenToKeys  +"){key = (event.which || event.keyCode);if (key==9||key==13){$(this).updateEditableCell(this)}else if(key==27){$(this).cancelEditableCell(this)}}' value='" + oldValue + "'></input>" +
            "<div class='input-group-append' data-target='#datepicker-appointment-date' data-toggle='datetimepicker'>" +
            "<div class='input-group-text'><i class='far fa-calendar-alt'></i></div>" +
            "</div>"
        input.html = startWrapperHtml

        setTimeout(function () { //Set timeout to allow the script to write the input.html before triggering the datepicker
            var self = jQuery('.date-time-picker').datetimepicker({locale: 'pt-BR'})
        },100);
        break;
        ...
        default: // text input
            listenToKeys = settings.listenToKeys;
            input.html = startWrapperHtml + "<input id='ejbeatycelledit' class='" + inputCss + "' onkeydown='if (" +listenToKeys +"){key=(event.which || event.keyCode);if (key==9||key==13){$(this).updateEditableCell(this)}else if(key==27){$(this).cancelEditableCell(this)}}' onfocusout='$(this).updateEditableCell(this)' value='" + oldValue + "'></input>" + endWrapperHtml;
            break;
    }
    return input;
}
